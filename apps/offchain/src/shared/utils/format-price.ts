...
export function formatAmountLong(amount?: number | string | null) {
  return formatToXyro(amount, 'long')
}

export function formatToXyro(
...
export const formatToUSD = (price?: Maybe<number>, precision = 2) => {
...
export const formatToTether = (price?: Maybe<number>, precision = 2) => {
  if (price === 0) return '0.00'
  if (!price) return null
