...
import {
  ...
  AttachTelegramMutation,
  AttachTwitterMutation
} from '__generated__/graphql'
import { MUTATION_ATTACH_TELEGRAM } from 'api/account/mutation-attach-telegram'
...
export const useAttachSocialNetwork = () => {
  ...
  const [attachTelegram, { loading: attachTelegramLoading }] =
    useMutation<AttachTelegramMutation>(MUTATION_ATTACH_TELEGRAM)
  ...
  const handleClearAttachData = () => {
    replaceUrlState(RouterPathes.accountSocials)
  ...
  const handleAttachTelegram = async () => {
    const telegramId = query.get('id') || 0
  ...
  useEffect(() => {
    isSocialNetworkAttachLoadingVar(
      (attachDiscordLoading || attachTwitterLoading || attachTelegramLoading) ??
        false
    )
  }, [attachDiscordLoading, attachTwitterLoading, attachTelegramLoading])
}
