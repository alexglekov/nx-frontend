...
import { AssetPrice, Maybe } from 'shared/types'
...
export const useChartUpdating = (
  ...
  useEffect(() => {
    ...
    const shouldUseBasePrice = isNotNullOrUndef(basePrice) && basePrice > 0
    const firstValue =
      shouldUseBasePrice ? nextToLastChartPrice?.value : lastChartPrice?.value
    const secondValue =
      shouldUseBasePrice ? basePrice : nextToLastChartPrice?.value

    const priceLineColor = getChartLineColorByPriceDiff(firstValue, secondValue)
...
const getUpdatingChartSeriesCallback = (
  seriesInstanceRef: MutableRefObject<ISeriesApi<'Area' | 'Baseline'> | null>,
